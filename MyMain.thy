...
datatype val = VInt int | VString string 
  | VBool bool | Person string | VList "val list"

datatype pair = Pair string "string \<Rightarrow> int"
...
function (sequential) findAtt :: "string \<Rightarrow> objectModel \<Rightarrow> (string \<Rightarrow> int)" where
"findAtt s (OM Nil) = doNothing" |
"findAtt s (OM (Cons (Pair x f) xs)) = 
(if x=s then (f) else (findAtt s (OM xs)))"
  by pat_completeness auto

fun assign :: "string \<Rightarrow> Person" where 
"assign s = P s"
...
"equalVal (Person p1) (Person p2) = (p1 = p2)" |
...
"exec (Select (MySQL.Col c)) om = VString ''null''" |
...
"exec (SelectFromWhere (MySQL.Col col) (FROM table) 
(WHERE (MySQL.Eq (MySQL.Col idCol) (MySQL.Var v)))) om
= VInt ((findAtt col om) v)" |
"exec (SelectFromWhere (MySQL.Eq (MySQL.Col col) (MySQL.Int i)) 
(FROM table) 
(WHERE (MySQL.Eq (MySQL.Col idCol) (MySQL.Var v)))) om
= VBool (equalVal (exec (SelectFromWhere (MySQL.Col col) (FROM table) 
(WHERE (MySQL.Eq (MySQL.Col idCol) (MySQL.Var v)))) om) (VInt i))" |
...
"eval (MyOCL.Att v col) om = VInt ((findAtt col om) v)"
...
= exec (SelectFromWhere (MySQL.Eq (MySQL.Col age) (MySQL.Int 30))
(FROM Lecturer)
(WHERE (MySQL.Eq (MySQL.Col lid) (MySQL.Var self)))) om"
proof auto
qed
end
